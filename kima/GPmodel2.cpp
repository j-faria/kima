...
#if TIMING
#include <fstream>
std::ofstream timing_file_GPmodel2;
#endif

/// set default priors if the user didn't change them
void GPmodel2::setPriors()  // BUG: should be done by only one thread!
{
    #if TIMING
    timing_file_GPmodel2.open("T_GPmodel2.txt");
    #endif
...
void GPmodel2::from_prior(RNG& rng)
{
    // preliminaries
    setPriors();
    save_setup();
...
void GPmodel2::calculate_mu()
{
    ...
    timing_file_GPmodel2 << "model_eval: " << took << endl;
    #endif
...
Eigen::MatrixXd GPmodel2::calculate_C() const
{
    size_t N = data.N();
    ...
    switch (kernel)
    {
        case standard:
        {
            K = calculate_C_quasiperiodic(data, eta1, eta2, eta3, eta4);
            break;
        }
        case qpc:
        {
            K = calculate_C_qpc(data, eta1, eta2, eta3, eta4, eta5);
            break;
        }
        case permatern32:
        {
            K = calculate_C_permatern32(data, eta1, eta2, eta3, eta4);
            break;
        }
        case permatern52:
        {
            K = calculate_C_permatern52(data, eta1, eta2, eta3, eta4);
            break;
        }
        case perrq:
        {
            K = calculate_C_perrq(data, eta1, eta2, eta3, eta4, alpha);
            break;
        }
        case periodic:
        {
            K = calculate_C_periodic(data, eta1, eta3, eta4);
            break;
        }
    ...
    // add jitter(s) to the diagonal and fill in the lower triangular part
    for (size_t i = 0; i < N; i++) {
        for (size_t j = i; j < N; j++) {
            if (i == j) {
                ...
                if (data.datamulti) {
                   ...
                   K(i, j) += sig * sig + jit * jit;
                }
                else {
                   K(i, j) += sig * sig + extra_sigma * extra_sigma;
                }
            }
            else {
                K(j, i) = C(i, j);
    ...
    // cout << took;
    // cout << " ns" << "\t"; // << std::endl;
    timing_file_GPmodel2 << "GP: " << took << endl;
    #endif
...
void GPmodel2::remove_known_object()
{
    double f, v, ti, Tp;
...
void GPmodel2::add_known_object()
{
    for (int j = 0; j < n_known_object; j++) {
        auto v = brandt::keplerian(data.t, KO_P[j], KO_K[j], KO_e[j], KO_w[j], KO_phi[j], data.M0_epoch);
        for (size_t i = 0; i < data.N(); i++) {
...
double GPmodel2::perturb(RNG& rng)
{
    #if TIMING
    auto begin = std::chrono::high_resolution_clock::now();  // start timing
    #endif
    ...
    auto end = std::chrono::high_resolution_clock::now();
    auto took = std::chrono::duration_cast<std::chrono::nanoseconds>(end-begin).count();
    timing_file_GPmodel2 << "perturb: " << took << endl;
    #endif
...
double GPmodel2::log_likelihood() const
{
    size_t N = data.N();
    ...
    // log-likelihood
    logL =
        -0.5 * y.size() * log(2 * M_PI) - 0.5 * logDeterminant - 0.5 * exponent;

    #if TIMING
    auto end = std::chrono::high_resolution_clock::now();
    auto took = std::chrono::duration_cast<std::chrono::nanoseconds>(end-begin).count();
    timing_file_GPmodel2 << "like: " << took << endl;
    #endif
...
void GPmodel2::print(std::ostream& out) const
{
    // output precision
...
string GPmodel2::description() const
{
    string desc;
    string sep = "   ";
