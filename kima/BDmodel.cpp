...
void BDmodel::setPriors()  // BUG: should be done by only one thread!
{
...
void BDmodel::from_prior(RNG& rng)
{
    // preliminaries
    setPriors();
...
void BDmodel::remove_known_object()
{
    double f, v, ti, Tp;
...
double BDmodel::perturb(RNG& rng)
{
    #if TIMING
    auto begin = std::chrono::high_resolution_clock::now();  // start timing
    #endif
...
*/
double BDmodel::log_likelihood() const
{
    size_t N = data.N();
...
void BDmodel::print(std::ostream& out) const
{
    // output precision
...
string BDmodel::description() const
{
    string desc;
    string sep = "   ";
    ...
    if (maxpl > 0) {
        ...
        for(int i = 0; i < maxpl; i++) desc += "L" + std::to_string(i) + sep;
    }
...
void BDmodel::save_setup() {
    ...
    if (planets.get_max_num_components()>0){
        ...
        fout << "K1prior: " << *conditional->K1prior << endl;
        fout << "K2prior: " << *conditional->K2prior << endl;
        fout << "eprior: " << *conditional->eprior << endl;
        fout << "phiprior: " << *conditional->phiprior << endl;
        fout << "wprior: " << *conditional->wprior << endl;
        fout << "Lprior: " << *conditional->Lprior << endl;
    }
