...
namespace DNest4
{
    ...
    Triangular::Triangular(double lower, double centre, double upper)
        : lower(lower), centre(centre), upper(upper)
    {
        if (upper < centre)
    ...
    void Triangular::setpars(double lo, double c, double up)
    {
        if (up < c)
            throw std::domain_error("Triangular distribution must have upper >= centre");

        if (lo > c)
            throw std::domain_error("Triangular distribution must have lower <= centre");

        if (up <= lo)
            throw std::domain_error("Triangular distribution must have lower < upper");
